...
export function renderProgramaciones(): string {
  ...
  return `
    ...
    <div class="prog-modal" id="modalConfirmarEliminacionProg" style="display:none;">
      <div class="prog-modal-overlay"></div>
      <div class="prog-modal-content" style="max-width:520px;">
        <div class="prog-modal-header">
...
export async function initProgramacionesEvents(): Promise<void> {
  ...
  window.addEventListener('abrirModalEliminarProg', ((e: CustomEvent) => {
    idEliminarPendiente = e.detail.id;
    const modal = document.getElementById('modalConfirmarEliminacionProg');
    if (modal) {
...
async function eliminarProg(id: number) {
  window.dispatchEvent(new CustomEvent('abrirModalEliminarProg', { detail: { id } }));
}
